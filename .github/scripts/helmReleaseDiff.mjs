...
async function helmTemplate(release, repository) {
  ...
  if ("type" in repository.spec && repository.spec.type == "oci") {
    manifests = await $`${helm} template --kube-version 1.27.2 --release-name ${
...
const currentRepository = await helmRepository(
  KubernetesDir,
...
const incomingRepository = await helmRepository(
  KubernetesDir,
